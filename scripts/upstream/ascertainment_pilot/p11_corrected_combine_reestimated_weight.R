...
base_dir <- file.path(root, "results/p11_scz_neff_audit/corrected_reestimated_weight_jackknife")
out_dir <- file.path(root, "results/p11_scz_neff_audit/corrected_reestimated_weight")
...
pairs <- data.frame(
  comparison = "SCZ_FINNGEN_minus_PGC",
  left = "SCZ_FINNGEN_R13",
  right = "SCZ_PGC2022",
  stringsAsFactors = FALSE
)

rows <- vector("list", nrow(pairs))
records <- readRDS(file.path(root, "results/p11_scz_neff_audit/corrected_p25_final/p25_phbc_records.rds"))
