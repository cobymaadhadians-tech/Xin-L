...
sample_rep <- 1000L
seed <- 20260831L
out_dir <- file.path(root, "results/p25/final")
...
# Assemble each family-fixed cache once.
base <- setNames(lapply(names(p25_panels), function(target) {
...
full_records <- setNames(lapply(names(base), function(target) fit_panel(base[[target]], "full_panel")), names(base))
