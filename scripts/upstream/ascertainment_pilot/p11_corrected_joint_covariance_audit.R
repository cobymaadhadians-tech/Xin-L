...
n_block <- 200L
out_dir <- file.path(root, "results/p11_scz_neff_audit/corrected_joint_covariance_audit")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

load_target <- function(target) {
  traits <- c(target, p25_panels[[target]])
  files <- sort(list.files(
    if (identical(target, "SCZ_PGC2022")) file.path(root, "results/p11_scz_neff_audit/phbc_cache") else file.path(root, "results/p25/phbc_cache", target),
